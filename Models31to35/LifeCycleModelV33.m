%% Life-Cycle Model 33: Portfolio-Choice with Warm-Glow of Bequests
% Warm-Glow of Bequests is subtle with Epstein-Zin preferences. We simply
% extend Life-Cycle Model 32 to include Warm-Glow of Bequests.

% To use warm-glow of bequests, you have to be using survival probabilities as a risk
vfoptions.survivalprobability='sj';

% Second, we set up a warm-glow of bequest function that takes (next period endogenous state)
% aprime as input (plus any parameters)
vfoptions.WarmGlowBequestsFn=@(aprime,wg,sigma,agej,J) LifeCycleModelV33_WarmGlowBequestsFn(aprime,wg,sigma,agej,J);
% This is essentially putting aprime into the utility function, and then
% also multiplying the whole thing by wg, which is thus a parameter that
...
Params.phi=5; % Additional risk aversion (from Epstein-Zin preferences)
...
ReturnFn=@(savings,a,z,w,sigma,agej,Jr,pension,kappa_j) ...
    LifeCycleModelV31_ReturnFn(savings,a,z,w,sigma,agej,Jr,pension,kappa_j);
